...
describe('credentials detection', () => {
  it('detects ANTHROPIC_API_KEY in env content', () => {
    ...
    const hasCredentials =
      /^(CLAUDE_CODE_OAUTH_TOKEN|ANTHROPIC_API_KEY|NANOCLAW_LLM_API_KEY)=/m.test(
        content,
      );
  ...
  it('detects NANOCLAW_LLM_API_KEY in env content', () => {
    const content = 'NANOCLAW_LLM_API_KEY=proxy-key-123';
    const hasCredentials =
      /^(CLAUDE_CODE_OAUTH_TOKEN|ANTHROPIC_API_KEY|NANOCLAW_LLM_API_KEY)=/m.test(
        content,
      );
